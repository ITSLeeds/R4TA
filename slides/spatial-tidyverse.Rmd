...
na = world %>%
  filter(continent == "North America") 
png('coord_compare.png', width = 1000, height = 250)
par(mfrow = c(1, 2), mar=c(0,0,0,0))
plot(na[0]); plot(na_2163[0])
dev.off()
```

![](http://www.robinlovelace.net/presentations/figs/coord_compare.png)

---

## tidyverse functions on OSM data
...
leaflet() %>% 
  addTiles() %>% 
  addPolylines(data = bristol_fast)
```
...
png('plot_compare.png', width = 800, height = 300)
par(mfrow = c(1, 2), mar=c(0,0,1,0))
plot(world[0]);plot(world["pop"])
dev.off()
```

![](http://www.robinlovelace.net/presentations/figs/plot_compare.png)
